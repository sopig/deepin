...
- (void)setCustomLoadingView{
    UIImageView * customImageView = [[UIImageView alloc] initWithFrame:CGRectMake(0, 0, 46, 46)];
    [customImageView setBackgroundColor:[UIColor clearColor]];
    [customImageView setImage:[UIImage imageNamed:@"circle_loading2.png"]];
    
    self.customView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, 46, 46)];
    [self.customView setBackgroundColor:[UIColor clearColor]];
    [self.customView addSubview:customImageView];
}
...
- (void)layoutSubviews {
    ...
    CGFloat lbwidth = labelSize.width;
	labelSize.width = MIN(labelSize.width, maxWidth);
	totalSize.width = MAX(totalSize.width, labelSize.width);
	
    
    //add by bobo 增加label换行显示功能
    if ( self.mode == MBProgressHUDModeText ) {
        if (lbwidth > maxwidth) {
            totalSize.height += ceil(lbwidth/maxwidth) *20.0;
        }
        else{
    ...
    if ( self.mode ==  MBProgressHUDModeText ) {
        ...
        if (lbwidth > maxwidth) {
            labelF.size = CGSizeMake(maxwidth, ceil(lbwidth/maxwidth)*20.0) ;
            //totalSize.height += (ceil(lbwidth/256) + 1) *20.0;
        }
    }
	label.frame = labelF;
